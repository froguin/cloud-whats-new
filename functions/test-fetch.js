...
export const testHandler = async () => {
  try {
    ...
    console.log(`기존 캐시 아이템 수: ${processedItems.length}`);
    ...
    // 최신 n개의 아이템 처리
    const n = MAX_ITEMS_TO_PROCESS; // 처리할 최대 아이템 수
    const recentItems = rss.items.slice(0, n);
    const processedItemsSet = new Set(processedItems.map(item => `${item.guid}|${item.pubDate}`));

    for (const item of recentItems) {
      const itemGuid = item.guid;
      const itemPubDate = new Date(item.published).toISOString();

      if (!processedItemsSet.has(`${itemGuid}|${itemPubDate}`)) {
        console.log('처리 시작:', item.title);
    ...
    console.log(`최종 캐시된 아이템 수: ${filteredItems.length}`);
    
    console.log('=== 테스트 함수 완료 ===');
  } catch (error) {
