<!--
  평가 기준(rubric)의 근거 문서.
  출처: https://github.com/snflkd/fluent-korean
        plugins/fluent-korean/output-styles/fluent-korean-not-coding.md (MIT License)
  이 서비스의 worker/index.js FLUENT_KOREAN_GUIDE 는 이 원문을 각색한 것이며,
  scripts/model-eval.mjs 의 품질 채점은 이 원문 지침을 기준으로 한다.
  README(upstream)의 요청대로 이 지침은 요약하지 않고 예시까지 원문 그대로 보존한다.
-->

# fluent-korean (not-coding) — 평가 기준 원문
